...
# Constitutional note: Neo4jAdapter is forbidden.  All Neo4j writes
# must go through Neo4jConnection -> governed_session -> GovernedNeo4jSession.
# The HAS_NEO4J flag is kept so callers can gate exports on connection availability.
HAS_NEO4J = True  # export_to_neo4j accepts Neo4jConnection at runtime
...
class UltraGraphBuilder:
    ...
    def build_graph(
        ...
        return {"nodes": list(self.nodes.values()), "edges": self.edges, "metrics": metrics, "build_time": build_time}
    ...
    def _process_entities(self, entities: List[Dict], source_id: Optional[str]):
        ...
        for entity in entities:
            ...
            if node_id in self.nodes:
                ...
                # Create new node
                # Validate label against governance allowlist
                try:
    ...
    # -----------------------------------------------------------------------
    # Schema allowlists for this builder (Patch Group C)
    # -----------------------------------------------------------------------
    _EXPORT_ALLOWED_NODE_PROPS = frozenset(
        {
            "article",
            "article_number",
            "branch",
    ...
    _EXPORT_RESERVED_NODE_KEYS = frozenset({"id", "label", "node_type", "confidence", "quality_score"})
    _EXPORT_ALLOWED_EDGE_PROPS = frozenset(
        {
            "contradiction_type",
            "description",
            "exception_type",
    ...
    _EXPORT_RESERVED_EDGE_KEYS = frozenset({"type", "weight", "confidence", "quality_score"})

    @classmethod
    def _validated_node_props(cls, properties: dict) -> dict:
        """Strip reserved keys and reject unknown ones fail-closed."""
        collisions = set(properties) & cls._EXPORT_RESERVED_NODE_KEYS
        if collisions:
            raise ValueError(f"Node properties override reserved keys: {sorted(collisions)}")
        unknown = set(properties) - cls._EXPORT_ALLOWED_NODE_PROPS
        if unknown:
            raise ValueError(f"Node properties contain unsupported keys: {sorted(unknown)}")
        return {k: v for k, v in properties.items() if k in cls._EXPORT_ALLOWED_NODE_PROPS}

    @classmethod
    def _validated_edge_props(cls, properties: dict) -> dict:
        """Strip reserved keys and reject unknown ones fail-closed."""
        collisions = set(properties) & cls._EXPORT_RESERVED_EDGE_KEYS
        if collisions:
            raise ValueError(f"Edge properties override reserved keys: {sorted(collisions)}")
        unknown = set(properties) - cls._EXPORT_ALLOWED_EDGE_PROPS
        if unknown:
            raise ValueError(f"Edge properties contain unsupported keys: {sorted(unknown)}")
        return {k: v for k, v in properties.items() if k in cls._EXPORT_ALLOWED_EDGE_PROPS}

    def export_to_neo4j(self, connection: object) -> bool:
        """
        Export graph to Neo4j via GovernedNeo4jSession.

        GOVERNANCE: all writes go through write_node / write_relationship.
        Raw Cypher, session.run(), and Neo4jAdapter are FORBIDDEN.
        """
        import uuid
        ...
        if not isinstance(connection, Neo4jConnection):
            raise TypeError(
                ...
                f"{type(connection).__name__}. Raw adapters are forbidden."
            )

        try:
            print("📤 Exporting graph to Neo4j (governed write surface)...")
            ...
            with connection.governed_session(
                ...
                # Write nodes
                for node in self.nodes.values():
                    clean_props = self._validated_node_props(node.properties)
                    node_data = {
                        "id": node.id,
                        "label": node.label,
                    ...
                    session.write_node(label="GraphNode", node_data=node_data, merge=True)

                # Write relationships
                for edge in self.edges:
                    clean_props = self._validated_edge_props(edge.properties)
                    rel_data = {
                        "type": edge.relationship_type,
                        "weight": edge.weight,
                        "confidence": edge.confidence,
