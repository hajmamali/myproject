...
class DeploymentProfile:
    ...
    def validate_model_compatibility(
        ...
        """
        return (
            model_size_gb <= self.resource_limits.max_model_size_gb and
            model_memory_gb <= self.resource_limits.max_memory_gb and
            model_context_window <= self.resource_limits.max_context_window
        )
    ...
    def get_optimal_model_size(self, available_models: List[str]) -> Optional[str]:
        ...
        compatible = [m for m in available_models if any(
            rec in m for rec in self.recommended_model_sizes
        )]
...
def validate_profile_compatibility(
    ...
    """
    return (
        required_memory_gb <= profile.resource_limits.max_memory_gb and
        required_cpu_cores <= profile.resource_limits.max_cpu_cores and
        required_model_size_gb <= profile.resource_limits.max_model_size_gb
    )
