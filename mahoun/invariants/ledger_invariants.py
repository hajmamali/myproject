...
LEDGER_INVARIANTS = [
    ...
    InvariantSpec(
        ...
        enforced_at=[
            "mahoun/ledger/privacy.py::filter_facts_for_ledger",
            "mahoun/reasoning/evidence_linked_verdict.py::generate_verdict",
        ],
    ...
    InvariantSpec(
        ...
        description="Automatic rejection of tombstoned evidence: previously deleted, redacted, or soft-deleted evidence to maintain absolute privacy and integrity. No resurrection of purged data.",
        enforced_at=[
...
def verify_el_i8_registration() -> bool:
    """Verify EL-I8 is properly registered in the invariant registry"""
    try:
        el_i8 = get_invariant_by_id("EL-I8")
        # Verify it has the expected metadata
        return (
            el_i8.id == "EL-I8"
            and el_i8.name == "Tombstone Security Guards"
            and "tombstone" in el_i8.description.lower()
            and "privacy" in el_i8.description.lower()
            and len(el_i8.enforced_at) >= 3
            and "privacy" in el_i8.failure_consequence.lower()
            and "gdpr" in el_i8.failure_consequence.lower()
        )
