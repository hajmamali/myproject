...
def load_optimizer_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    defaults = {"checkpoint_interval": 10, "report_dir": "logs/graph_optimization", "enable_feedback_loop": True}
...
async def run_optimization_cycle(
    ...
    try:
        ...
        async with GovernanceContextManager.active_context(correlation_id=correlation_id, actor_id=actor_id) as ctx:
            with connection.governed_session(correlation_id=correlation_id, actor_id=actor_id) as session:
                ...
                receipt = session.write_node(
                    "OptimizationJob",
                    {
                        "id": correlation_id,
                        "status": "running",
                        "provenance": ctx.require_provenance(source="optimizer", author=actor_id).to_dict(),
                    },
                )
        ...
        # PATCH GROUP A / F: route through Neo4jConnection.governed_session()
        # instead of constructing GovernedNeo4jSession directly.
        # This preserves the constitutional chain:
        #   GovernanceContextManager -> Neo4jConnection -> governed_session
        # Direct GovernedNeo4jSession construction bypassed that chain.
        def session_factory() -> GovernedNeo4jSession:
            with connection.governed_session(
                correlation_id=correlation_id,
                actor_id=actor_id,
            ) as session:
                return session  # caller holds the context-managed session
...
def main():
    ...
    parser.add_argument("--config", type=str, default=None, help="Path to optimizer config YAML file")
    parser.add_argument("--report-dir", type=str, default=None, help="Directory for optimization reports")
