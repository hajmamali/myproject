...
class LegalCypherQueries:
    ...
    FIND_SUPERSEDED_LAWS = CypherQuery(
        ...
        WHERE law.id = $law_id
        AND law._deleted IS NULL AND newer_law._deleted IS NULL
        RETURN
        ...
        """,
        parameters={"law_id": "ID of the law to check for supersession"},
    ...
    FIND_SUPERSESSION_CHAIN = CypherQuery(
        ...
        LIMIT 1
        """,
        parameters={"law_id": "ID of any law in the supersession chain"},
        returns="Complete supersession chain from oldest to newest law",
        complexity="MEDIUM",
        use_cases=["Legal history analysis", "Finding current active version of a law", "Supersession audit trails"],
    )
    ...
    VALIDATE_NO_SUPERSESSION = CypherQuery(
        ...
        WHERE law._deleted IS NULL
        OPTIONAL MATCH (law)-[:SUPERSEDED_BY]->(newer_law:Law)
        WHERE newer_law IS NULL OR newer_law._deleted IS NULL
        RETURN
            law.id as law_id,
        ...
        """,
        parameters={"law_id": "ID of the law to validate"},
        returns="Validation result and superseding law information if applicable",
        complexity="LOW",
        use_cases=["Pre-retrieval validation", "Legal document filtering", "Zero-hallucination compliance"],
    )
    ...
    RANK_BY_COURT_HIERARCHY = CypherQuery(
        ...
        LIMIT $limit
        """,
        parameters={"case_type": "Type of case to search for", "limit": "Maximum number of results to return"},
        returns="Verdicts ranked by court hierarchy and authority score",
        complexity="MEDIUM",
        use_cases=["Legal precedent ranking", "Authority-based document retrieval", "Court hierarchy enforcement"],
    )
    ...
    FIND_HIGHER_COURT_PRECEDENTS = CypherQuery(
        ...
        AND lower_verdict._deleted IS NULL AND higher_verdict._deleted IS NULL
        ...
        ORDER BY
            ...
            END ASC
        """,
        parameters={"case_number": "Case number to find higher court precedents for"},
        returns="Higher court precedents that may override lower court decisions",
        complexity="HIGH",
        use_cases=["Precedent hierarchy validation", "Legal contradiction detection", "Authority-based filtering"],
    )
    ...
    FILTER_ACTIVE_DOCUMENTS = CypherQuery(
        ...
        complexity="MEDIUM",
        use_cases=["Legal document retrieval", "Active law filtering", "Zero-hallucination compliance"],
    )
    ...
    CHECK_DOCUMENT_VALIDITY = CypherQuery(
        ...
        RETURN
            ...
            doc._deleted IS NOT NULL as is_tombstoned,
            ...
            CASE
                WHEN doc._deleted IS NOT NULL THEN false
        ...
        """,
        parameters={"document_id": "ID of the document to check"},
        returns="Comprehensive validity status with reasons for invalidity",
        complexity="MEDIUM",
        use_cases=["Document validity verification", "Legal compliance checking", "Audit trail generation"],
    )
    ...
    FIND_CITATION_NETWORK = CypherQuery(
        ...
        WHERE center_doc._deleted IS NULL
        OPTIONAL MATCH (center_doc)-[:CITES]->(cited_doc)
        WHERE cited_doc IS NULL OR cited_doc._deleted IS NULL
        OPTIONAL MATCH (citing_doc)-[:CITES]->(center_doc)
        WHERE citing_doc IS NULL OR citing_doc._deleted IS NULL
        RETURN
        ...
        """,
        parameters={"document_id": "ID of the central document"},
        returns="Citation network with incoming and outgoing citations",
        complexity="MEDIUM",
        use_cases=["Citation analysis", "Authority score calculation", "Legal network analysis"],
    )
    ...
    CALCULATE_AUTHORITY_SCORE = CypherQuery(
        ...
        """,
        parameters={"document_id": "ID of the document to calculate authority score for"},
        returns="Calculated authority score based on citation patterns",
        complexity="HIGH",
        use_cases=["Authority score calculation", "Document ranking", "Citation-based filtering"],
    )
    ...
    RESOLVE_TEMPORAL_CONFLICTS = CypherQuery(
        ...
        AND doc1._deleted IS NULL AND doc2._deleted IS NULL
        ...
        """,
        parameters={"document_id_1": "ID of first document", "document_id_2": "ID of second document"},
        returns="Temporal precedence resolution between two documents",
        complexity="MEDIUM",
        use_cases=["Conflict resolution", "Temporal precedence enforcement", "Legal contradiction handling"],
    )
    ...
    GENERATE_RETRIEVAL_AUDIT_TRAIL = CypherQuery(
        ...
        RETURN
            ...
            doc.authority_score as authority_score,
            CASE
                WHEN doc._deleted IS NOT NULL THEN 'EXCLUDED: Document is soft-deleted (tombstoned)'
        ...
        """,
        parameters={"document_id": "ID of the document being audited"},
        returns="Complete audit trail for retrieval decision",
        complexity="HIGH",
        use_cases=["Regulatory compliance", "Audit trail generation", "Decision transparency"],
    )
    ...
    UPDATE_DOCUMENT_METADATA = CypherQuery(
        ...
        complexity="LOW",
        use_cases=["Legal metadata migration", "Document metadata updates", "Schema enhancement"],
    )
    ...
    CREATE_LEGAL_RELATIONSHIPS = CypherQuery(
        ...
        WHERE source._deleted IS NULL AND target._deleted IS NULL
        ...
        complexity="MEDIUM",
        use_cases=["Creating supersession relationships", "Establishing citation links", "Building legal hierarchies"],
    )
    ...
    VALIDATE_CROSS_SYSTEM_SYNC = CypherQuery(
        ...
        LIMIT $limit
        """,
        parameters={"limit": "Maximum number of documents to validate"},
        returns="Document metadata for cross-system validation",
        complexity="MEDIUM",
        use_cases=["Cross-system synchronization validation", "Data consistency checks", "Migration verification"],
    )
...
class LegalQueryExecutor:
    ...
    * Zero raw ``driver.session()`` calls exist in this class.
    ...
    def __init__(self, connection: object):
        ...
        """
        from mahoun.graph.neo4j.connection import Neo4jConnection as _Neo4jConnection

        if not isinstance(connection, _Neo4jConnection):
            raise TypeError(f"connection must be Neo4jConnection instance, got {type(connection).__name__}")

        self.connection = connection
        self.query_stats = {"total_queries": 0, "successful_queries": 0, "failed_queries": 0, "queries_by_category": {}}

    async def execute_legal_query(
        self, query_name: str, parameters: Dict[str, Any], timeout: int = 30
        ...
        * Mutation queries → ``connection.governed_session()`` (full audit trail)
        ...
        try:
            ...
            if is_mutation:
                # PATCH GROUP B/D: Free-form mutation Cypher is constitutionally
                # forbidden.  LegalCypherQueries stores pre-validated READ queries
                # only.  Any mutation must be implemented via typed governed
                # write operations (GovernedNeo4jSession.write_node /
                # write_relationship / delete_node).  Executing raw mutation
                # Cypher through this executor bypasses the ValidatorPipeline,
                # the ontology enforcer, and the audit ledger.
                ...
                raise GovernanceViolationError(
                    GovernanceViolation(
                        ...
                        message=(
                            f"Legal query '{query_name}' contains mutation Cypher "
                            f"which is forbidden in LegalQueryExecutor. "
                            f"Implement mutations via typed GovernedNeo4jSession "
                            f"write operations, not free-form Cypher."
                        ),
                        details={"query_name": query_name, "category": query.category.value},
                        source="LegalQueryExecutor.execute_legal_query",
                    )
                )
            else:
                # Read-only path: use execute_query (MutationAuthorizationBoundary
                # will still block any mutation that slips through _is_mutation_query)
    ...
    def _is_mutation_query(self, cypher: str) -> bool:
        ...
        mutation_keywords = ["CREATE ", "MERGE ", "SET ", "DELETE ", "REMOVE ", "DETACH DELETE"]
