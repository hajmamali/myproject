...
async def test_explicit_failed_validation():
    ...
    async with GovernanceContextManager.active_context(correlation_id="test_explicit_fail") as gov_ctx:
        ...
        assert len(blockchain.blockchain) >= 1, "Entry must be in ledger"
        
        # Find our entry in the blockchain
        found = False
        for block in blockchain.blockchain:
            if hasattr(block, 'verdict_id') and block.verdict_id == committed_entry.verdict_id:
                found = True
                assert block.validation_status == "FAILED", \
...
async def test_determinism_three_executions():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # VERIFY: All three executions have the same case_id
        case_ids = [r.verdict.case_id for r in results if r and r.verdict]
